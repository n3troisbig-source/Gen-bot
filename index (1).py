...
def cmd_setfeedbackchannel(interaction):
    """[Owner] Set the channel where /feedback submissions are posted."""
    uid = int(get_user(interaction)["id"])
    if not is_owner(uid):
        return msg("You do not have permission.", ephemeral=True)
    channel_id = get_option(interaction["data"].get("options", []), "channel")
    set_feedback_channel(channel_id)
    log_to_channel(f"📋 /setfeedbackchannel — set to <#{channel_id}>")
    return msg(f"✅ Feedback channel set to <#{channel_id}>.", ephemeral=True)


def cmd_feedback(interaction):
    """Any user can submit feedback. Posts to the configured feedback channel."""
    user      = get_user(interaction)
    uid       = int(user["id"])
    uname     = user.get("global_name") or user.get("username") or f"User#{user.get('discriminator','0')}"
    avatar    = f"https://cdn.discordapp.com/avatars/{user['id']}/{user.get('avatar','')}.png" if user.get("avatar") else "https://cdn.discordapp.com/embed/avatars/0.png"
    opts      = interaction["data"].get("options", [])
    feedback  = get_option(opts, "message") or ""

    if not feedback.strip():
        return msg("❌ Please include a feedback message.", ephemeral=True)

    # Build the feedback embed for the staff channel
    ...
    feedback_embed = {
        ...
        "footer": {"text": f"Submitted • {now_str}", "icon_url": avatar},
        "timestamp": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())
    }

    # Post to feedback channel if configured
    fb_ch = get_feedback_channel()
    if fb_ch:
        post_to_channel(fb_ch, embeds=[feedback_embed])
    else:
        # fallback: post to log channel
        ch = LOG_CHANNEL_ID or _jget("r6_log_channel", "")
        if ch:
            post_to_channel(ch, embeds=[feedback_embed])
    ...
    return msg(embeds=[reply_embed], ephemeral=True)
...
COMMANDS = {
    ...
    "setfeedbackchannel": cmd_setfeedbackchannel, "feedback": cmd_feedback,
}
